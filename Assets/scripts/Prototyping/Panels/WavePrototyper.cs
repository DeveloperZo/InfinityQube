...
public class WavePrototyper : PrototypingPanelBase
{
    ...
    private void DrawPairedWaveSection()
    {
        DrawSection("", () =>
        {
            ...
            {
                GUILayout.Label($"  Light: {markers.lightMarkerPositions.Count} → Unit cubes");
                GUILayout.Label($"  Heavy: {markers.heavyMarkerPositions.Count} → Recursion cubes");
                GUILayout.Label($"  Prime: {markers.primeMarkerPositions.Count} → Prime cubes");
                GUILayout.Label($"  Infinity: {markers.infinityMarkerPositions.Count} → Infinity cubes");
            }
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Light", GUILayout.Width(50))) PlaceTestMarker(MarkerMode.Light);
            if (GUILayout.Button("Heavy", GUILayout.Width(50))) PlaceTestMarker(MarkerMode.Heavy);
            if (GUILayout.Button("Prime", GUILayout.Width(50))) PlaceTestMarker(MarkerMode.Prime);
            if (GUILayout.Button("Clear", GUILayout.Width(50))) ClearRecordedMarkers();
            GUILayout.EndHorizontal();
            ...
            {
                PlaceTestMarker(MarkerMode.Light);
                PlaceTestMarker(MarkerMode.Light);
                PlaceTestMarker(MarkerMode.Heavy);
            }
